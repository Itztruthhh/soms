...
bot = telebot.TeleBot('7646347378:AAHdXnLToR797_G_qj70jgcZF7KoXBXZ5Qo')
...
def start_attack_reply(message, target, port, time):
    ...
    # Dictionary to store the last time each user ran the /filzer command
bgmi_cooldown = {}
...
@bot.message_handler(commands=['filzer'])
    ...
    if user_id in allowed_user_ids:
        ...
        if len(command) == 4:  # Updated to accept target, port, and time
            ...
            if time > 300:
                ...
                try:
                    record_command_logs(user_id, '/filzer', target, port, time)
            ...
            response = "Usage: /filzer <target> <port> <time>"
    else:
...
def show_help(message):
    help_text ='''
💥 /filzer : 😫BGMI WALO KI MAA KO CHODO🥵. 
